...
import { leaveConfirmMessage, useSolveRun, type Stage } from './_lib/useSolveRun';
...
export default function SolvePage() {
  ...
  // ── THE STAGE MACHINE ──
  // The Stage union, every transition, the recording boundary and the
  // leave guards live in useSolveRun (./_lib/useSolveRun). Destructured
  // rather than held as an object so every reference below reads exactly
  // as it did when this was all one file.
  //
  // CALLED HERE, not lower down, for one reason that matters: the run-plan
  // effect below closes over `setStage`, and every handler in the render
  // reads `stage`. The two derivations above are the hook's only inputs
  // from this page that are not already in scope at this point.
  const {
    stage,
    setStage,
    recordingFailure,
    ...
    runAtRisk,
    finishRecording,
  } = useSolveRun({ recorder, scramble, unfiledCount, filingFailed });

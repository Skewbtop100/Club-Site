...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { cloudinaryDeleted, cloudinaryDetail, stillsDeleted, stillsFailed, stillsRefused, refused, r2Deleted, r2Detail } =
  ...
  return NextResponse.json({
    ...
    r2Deleted,
    r2Detail,
    // Assets the document named that were not deleted because they were not
    // provably its own — see submission-cleanup.ts.
    stillsRefused,
    refused,
  });
}

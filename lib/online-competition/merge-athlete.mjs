...
//
//   pre-flight -> C,E (field rewrites) -> B (registrations) -> F,G (shared
//   containers) -> A (profile, last)
//
// There is no D any more. It moved onlineSeasonPoints documents; season
// points were removed, and their stored data is left where it is — a
// merged athlete's old season docs stay under the old uid, unread. The
// letters are not renumbered, so logs and counts keep their meaning.
//
 ...
 *  (no submissions, no registrations, no profile) — all three of which a
 *  tombstoned document passes, by construction. */
...
export async function planMerge(db, { oldEmail, newEmail, resume = false }) {
  ...
  return {
    ...
    errors,
    resumeUsed: resume,
    newAccountState: { submissions: newSubs, registrations: newRegs, profileStatus: newStatus ?? null },
  };
}
...
export async function commitMerge(db, planned) {
  ...
  const counts = { A: 2, B: 0, C: 0, E: 0, F: 0, G: 0 };

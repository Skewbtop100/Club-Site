...
export async function planMerge(db, { oldEmail, newEmail, resume = false }) {
  ...
  const checks = [
    ...
    { code: MERGE_ERRORS.ALREADY_MERGED, ok: oldData.mergedInto === undefined, label: 'old doc has not already been merged' },
  ];
...
export async function commitMerge(db, planned) {
  ...
  await newRef.set(mergedScalars, { merge: true });
  if (oldStats !== undefined) await newRef.update({ stats: oldStats });
  await oldRef.update({
    mergedInto: newUid,
    mergedAt: FieldValue.serverTimestamp(),
    profileStatus: FieldValue.delete(),
  });

...
export interface Competition {
  ...
  /** True only for the singleton "daily-practice" pseudo-competition (see
   *  lib/firebase/services/competitions.ts). Every list/stat that treats
   *  `competitions` as "real club competitions" must exclude docs with
   *  this flag set. */
...
export interface Result {
  ...
  /** Club-local YYYY-MM-DD this was entered on. Only set for Daily Practice
   *  results (competitionId === DAILY_PRACTICE_COMPETITION_ID) — lets the
   *  live feed and the "already recorded today" check query by day without
   *  parsing the docId. */
  practiceDate?: string;
  /** Per-solve penalty, parallel to `solves`. `solves[i]` already has any +2
   *  baked in (e.g. a stored 1011 could be a raw 8.11 +2, or a genuine flat
   *  10.11) — this is what makes that distinction recoverable when editing.
   *  Optional because older results were saved before this field existed;
   *  DNF is always unambiguous either way (solves[i] === -1). */
  penalties?: ('none' | '+2' | 'dnf')[];
  /** Per-solve WCA-quality scramble (server-side, via cstimer_module —
   *  csTimer's own scrambler, see app/api/scramble/route.ts), parallel to
   *  `solves`. Daily Practice only — real-competition entry doesn't
   *  populate this, since admins there transcribe times against an actual WCA
   *  delegate's official scramble, not one generated by this app. Stored for
   *  transparency/verification, not shown on the public feed. */
  scrambles?: string[];
}
